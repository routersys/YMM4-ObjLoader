...
{
    public static class MmdSolver
    {
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static unsafe void ApplyImpulse(ref MmdPhysicsState state, Vector3 impulse, Vector3 relPos)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static void ApplyAngularImpulse(ref MmdPhysicsState state, Vector3 impulse)
        {
            if (state.InverseMass <= 0f) return;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static void SolveContact(ref MmdPhysicsState stateA, ref MmdPhysicsState stateB, ref MmdContactPoint contact, float dt, float erpMultiplier)
        {
            ...
            {
                float denom = MmdPhysicsMath.ComputeImpulseDenominator(
                ...
                float denomF1 = MmdPhysicsMath.ComputeImpulseDenominator(
                    invMassA, invIA, relPosA,
                    invMassB, invIB, relPosB, contact.FrictionDir1);
                contact.FrictionMass1 = denomF1 > 1e-8f ? 1f / denomF1 : 0f;

                float denomF2 = MmdPhysicsMath.ComputeImpulseDenominator(
                    invMassA, invIA, relPosA,
        ...
        public static void WarmStartContact(ref MmdPhysicsState stateA, ref MmdPhysicsState stateB, ref MmdContactPoint contact)
        {
            bool aStatic = stateA.InverseMass <= 0f;
            bool bStatic = stateB.InverseMass <= 0f;
            if (aStatic && bStatic) return;
        ...
        public static void SolveJointConstraint(
            ref MmdPhysicsState stateA, ref MmdPhysicsState stateB, PmxJoint joint,
            Vector3 localAnchorA, Vector3 localAnchorB,
        ...
        private static void SolveAxisLinearLimit(
            ref MmdPhysicsState stateA, ref MmdPhysicsState stateB, Vector3 axis, float error,
        ...
        {
            float invM = MmdPhysicsMath.ComputeImpulseDenominator(
            ...
            ref Vector3 angularImpulse, ref Vector3 springAngularImpulse)
        {
            ...
            var euler = MmdPhysicsMath.QuaternionToEuler(relRot);
